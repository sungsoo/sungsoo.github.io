...
In the above diagram, a chunk of neural network, \$A\$, looks at some input \$x_t\$ and outputs a value \$h_t\$. A loop allows information to be passed
...
work. They work tremendously well on a
...
called the "forget gate layer." It looks at \$h_{t-1}\$ and \$x_t\$, and outputs a number between \$0\$ and \$1\$ for
each number in the cell state \$C_{t-1}\$.
A \$1\$ represents "completely keep this"
while a \$0\$ represents "completely get rid
of this."
...
creates a vector of new candidate values, \$\\tilde{C}_t\$, that could be added to the
...
It's now time to update the old cell state, \$C_{t-1}\$, into the new cell state \$C_t\$. The previous steps already decided what
to do, we just need to actually do it.

We multiply the old state by \$f_t\$,
forgetting the things we decided to forget earlier. Then we add \$i_t*\tilde{C}_t\$. This is the new candidate
...
going to output. Then, we put the cell state through \$\tanh\$ (to push the values to be between \$-1\$ and \$1\$) and
