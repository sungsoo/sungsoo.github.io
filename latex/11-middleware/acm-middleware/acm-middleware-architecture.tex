...
\noindent
\textbf{Fundamental operations:} The CA provides the following key operations for multiscreen-based collaboration services.
\bi
...
\ii \textit{Remote execution:} User can execute the certain remote app at remote devices in the same network using a source device.
For example, if  a user want to execute the travel video app in remote set-top, a user can execute the remote travel video app via remote invocation using user's tablet. This function is useful to control diverse devices effectively.
