...
The $n$-screen device $ND$ is a wireless host which includes the NSAL and the collaboration agent. 
...
The physical application is an application running on a single device. 
This physical application lifecycle is the same as the android application and activity lifecycle.

%Collaboration Session
The collaboration session is defined as an container object that can be synchronized the associated information through the more than one different physical applications at runtime.
%Representation
%Inter-application relation schema
Decoupling the collaboration information from the multiscreen applications requires representing the inter-application (or \textit{inter-app}) relationship to support the collaboration service. The CA includes a manager for describing the inter-app relation among the associated apps called \textit{inter-app relation manager}. This manager uses an XML-based language to encode the necessary information for finding, executing and collaborating among the associated apps.
This inter-app relation information is stored on the $n$-screen service cloud. The inter-app relation manager in the CA periodically updates the information through the RESTful API.
