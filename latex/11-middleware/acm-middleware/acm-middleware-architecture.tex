...
\conferenceinfo{\textit{MidDep 2014,}}{December 8-12, Bordeaux, France}
\CopyrightYear{2014} % Allows default copyright year (20XX) to be over-ridden - IF NEED BE.
%\crdata{0-12345-67-8/90/01}  % Allows default copyright data (0-89791-88-6/97/05) to be over-ridden - IF NEED BE.
% --- End of Author Metadata ---

\title{A Collaboration Middleware in Multiscreen Environments}
...
\cite{Aarts:2004, Sadri:2011, Nandakumar:2014, Anstead:2014, Longo:2013, Motti:2013, DBLP:SchmohlB08, DBLP:FuentesPCM06, samsung:2014, Barkhuus:2009, Ma:2008, Lucent2011}. 
...
\noindent
\textbf{Collaboration session contruction:}  Figure \ref{fig:constructsession} shows an example workflow for collaboration session construction. Consider each $n$-screen device ($ND_1$, $ND_2$, $ND_3$)  exists in the same network environment. First, user executes an $n$-screen app, $A_1$, in the $ND_1$. If the app is successfully started, NSAL creates an initial collaboration session as depicted in Figure \ref{fig:constructsession}(a). In order to perform the \textit{remote execution}, $A_1$ requests service discovery to CA.  The service discovery function can be called with service properties as parameters, such as, $n$-screen device or $n$-screen apps. Then $A_1$ invites the other app, $A_2$, in $ND_2$ through \textit{session invitation} function. CA updates the collaboration session after session invitation is completed. If another app, $A_3$, in $ND_3$ want to join the collaboration session by using \textit{session join} function, CA adds $A_3$'s logical app information to the collaboration session. Finally, the collaboration session, which includes $A_1$, $A_2$, and $A_3$, is constructed as shown in Figure \ref{fig:constructsession}(b). 
...
Figure \ref{fig:pushmigration} shows an example workflow for collaboration session maintenance. 
...
%
\section{Experiments}
In this section, we evaluate the feasibility and the performance of our system in terms of service discovery for the collaboration services. 
...
We have implemented the CA and the NSAL in Java on Android platform. 
We conducted a series of experiments for our middleware performance on the multiscreen service platform.
In order to test the performance of our system, we used two smart set-tops (Intel Core i7, 8G RAM) which were connected to a 100 Mbps switch via a wireless network connection. Also, the three smart devices as mobile-clients (Intel 2GHz, 1G RAM) were connected to the wireless network. 
%These laptops are capable of H.264 decoding and displaying the game videostream as shown in Fig. \ref{fig:testenv}.
...
\section{Related Work}
\label{sc:RelatedWork}
In this section, we give a brief overview of related work on the multiscreen IPTV, second screen and multiscreen SDKs.
...
\noindent
\textbf{Multiscreen SDK:}
The multiscreen SDK provides developers with a set of APIs and tools that optimize multiscreen experiences for various applications, such as, games, media sharing, social collaboration, and so on.
...
However, this product supports  asymmetric collaboration since this SDK requires a smart TV to provide a collaboration service among associated apps.

\noindent
\textbf{Multiscreen IPTV:}  
Many content delivery platforms are developed in order to provide adaptive content according to screen properties of smart devices \cite{Longo:2013, Lucent2011}.
...
This article investigates the changing television watching practices amongst early adopters of personal hard-disk video recorders and Internet downloading of video.
