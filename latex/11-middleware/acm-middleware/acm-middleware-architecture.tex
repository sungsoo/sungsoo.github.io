...
The collaboration session is defined as an container object that can be synchronized the associated information through the more than one $n$-screen devices at runtime.
Representation

Inter-application relation schema

Decoupling the collaboration information from the multiscreen service  system requires representing the inter-application (or \textit{inter-app}) relationship to support the collaboration service. The CA includes a manager for describing the inter-app relation among the associated apps called \textit{inter-app relation manager}. The manage uses an XML-based language to encode the necessary information for finding, executing and collaborating among the associated apps.
